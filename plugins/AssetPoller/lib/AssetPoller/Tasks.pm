...
PATH:
    ...
    FILE:
        foreach my $f (@files) {
            ...
            my $file_filter
                = $p->get_config_value( 'file_filter', 'blog:' . $blog_id );
            if ($file_filter) {

	      # Remove any space before or after the comma separator, 
	      # and build an array.
            ...
            my $import_updated = $p->get_config_value( 'import_updated_files',
                'blog:' . $blog_id );
            if ($import_updated) {
		...
		# Use class "*" so that images, files, and 
		# any other class of object are returned.
                $asset
                    = MT->model('asset')
                    ->load(
                    { blog_id => $blog_id, class => '*', file_name => $f } );
                if ($asset) {
                    my $existing_stamp
                        = ts2epoch( $blog, $asset->modified_on );

                    # Only proceed if the file is newer than the asset.
                    next if ( $file_stamp <= $existing_stamp );

		    # The file is newer than the existing asset. 
		    # So, just import it.
            ...
            MT->log(
                {   level   => MT->model('log')->INFO(),
                    ...
                    message => MT->translate(
                        'Asset Poller has imported the file [_1].', $f,
                    ),
                }
            );
